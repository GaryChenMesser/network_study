...
def search_list_by_keyword(youtube, keyword, nextpage):
...
def video_tags_by_id(youtube, videoid):
  ...
  return response["id"], response["snippet"]["tags"]
...
class VideoGraph:
  ...
  def _similarity(self, a, b):
    return len(set(v_tag[a]).interaction(v_tag[b]))
    
  def _add_edge(self, index):
    for node in self.G.vertices():
      if _similarity(index, node) >= THRESHOLD:
...
if __name__ == "__main__":
  ...
  for i in range(1):
    nextpage = None
    for search in range(int(args.size) // 50 + 1):
      response = search_list_by_keyword(youtube, args.query, nextpage)
