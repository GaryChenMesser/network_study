...
#-----------------shared epidemic method----------------------------  
  def _update_state(self, flag):
    ...
    for i in range(2):
      if len(self.infected_list[i]) > 0:
        recover_list = {v : None for v in self.g.vertices() if np.random.random() < delta}
        
        temp1 = list(self.infected_list[i].keys())
        temp2 = list(self.infected_list[(i + 1) % 2].keys())
        
        for v in temp1:
          for neighbor in self.g.vertex(v).out_neighbors():
            if np.random.random() < e_beta[i][self.g.edge(v, neighbor)] and neighbor not in temp2:
