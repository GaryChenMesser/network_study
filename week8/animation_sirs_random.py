...
def sample_k(max):
    k = poisson(max)
    return k
g = random_graph(200, lambda: sample_k(6), model="probabilistic-configuration",
                     edge_probs=lambda i, k: 1.0 / (1 + abs(i - k)), directed=False,
                     n_iter=100)

pos = graph_draw(g, edge_color="black", output="randommodel.pdf")
...
if not offscreen:
    ...
    win2 = GraphWindow(g, pos, geometry=(500, 400),
                      edge_color=[0.6, 0.6, 0.6, 1],
                      vertex_fill_color=state,
                      vertex_halo=newly_infected,
                      vertex_halo_color=[0.8, 0, 0, 0.6])
